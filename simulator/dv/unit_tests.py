...
#from dv.router import DVRouter
#from dv.router_ref1 import DVRouter
#from dv.router_ref2 import DVRouter

from dv.dv_base import Table, TableEntry, DVRouterBase, PortCollection, FOREVER
from dv.dv_base import AdvertisementPacket as RoutePacket

class Route(namedtuple("RouteAd", ["dst", "latency"])):
    """Helper class for checking route advertisements."""

    def __repr__(self):
        return "{}(destination={}, latency={})".format(
            type(self).__name__,
...
class TestDVRouterBase(unittest.TestCase):
    ...
    def _advertise (self, route_dst, route_latency, port):
        p = RoutePacket(route_dst, route_latency)
        oldexpire = self.router.expire_routes
        self.router.handle_rx(p, port)
        self.router.expire_routes = oldexpire

    def _set_up(self, poison_reverse, poison_expired, poison_on_link_down, split_horizon, send_on_link_up, triggered=False):
        #TODO: Check for calls to time.time()

        # Simulate time.
        self._current_time = 50
        sim.api.current_time = lambda: self._current_time

        DVRouterBase._DVRouterBase__POISON_REVERSE = poison_reverse
        DVRouterBase._DVRouterBase__POISON_EXPIRED = poison_expired
        DVRouterBase._DVRouterBase__POISON_ON_LINK_DOWN = poison_on_link_down
        DVRouterBase._DVRouterBase__SPLIT_HORIZON = split_horizon
        DVRouterBase._DVRouterBase__SEND_ON_LINK_UP = send_on_link_up
        DVRouterBase._DVRouterBase__DEFAULT_TIMER_INTERVAL = 5
        DVRouterBase._DVRouterBase__DROP_HAIRPINS = False

        def _get_underlying_dict (this):
          return dict(this.up)
        PortCollection.get_underlying_dict = _get_underlying_dict

        DVRouterBase.start_timer = lambda *args,**kw: None
        self.router = DVRouter()
        assert not (self.router.SPLIT_HORIZON and self.router.POISON_REVERSE), "Split horizon and poison reverse can't both be on"
    ...
    def _patch_send(self, all_ports):
        ...
        old_send = self.router.send
        self.router.send = _send

        try:
          yield sent_packets
        finally:
          self.router.send = old_send
    ...
    def _ensure_no_send(self, all_ports):
        ...
        for port, packets in sent_packets.items():
            ...
            results.append("unexpected packet(s) sent out port {}: {}".format(
                    port, packets))
    ...
    def _add_test_routes_raw(self):
        ...
        self.router.table.update({
            h1: TableEntry(destination=h1, port=1, latency=8, expire_time=115),
            h2: TableEntry(destination=h2, port=3, latency=5, expire_time=109),
        })
    ...
    def _add_test_routes_proper(self):
        ...
        self._advertise(h1, port=1, route_latency=3)
        self._set_current_time(96)
        self._advertise(h2, port=1, route_latency=6)
        self._set_current_time(98)
        self._advertise(h1, port=3, route_latency=10)
        self._set_current_time(100)
        self._advertise(h2, port=3, route_latency=2)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=8, expire_time=94+self.router.ROUTE_TTL),
            h2: TableEntry(destination=h2, port=3, latency=5, expire_time=100+self.router.ROUTE_TTL),
        })

    def _compute_dict_diff(self, d, expected, singular, plural, ignore_extra=False):
        message = ""
        failed = False
    ...
    def _assert_route_ads_sent(self, sent, expected_sets, ignore_extra=False):
        """
        ...
        if failed:
            ...
            pass # Bug check below won't work in this case
        else:
            self.assertDictEqual(sent, expected, "BUG: dicts should be equal")
...
class TestForwarding(TestDVRouterBase):
    ...
    table, and forwarding table configurations.  The current time starts
    off at 100.
    """
    SCORE = 12
    ...
    def test_forward(self):
        ...
        with self._patch_send(all_ports={1, 2, 3}) as sent_packets:
            ...
            self.router.handle_rx(packet1, in_port=2)
        ...
        with self._patch_send(all_ports={1, 2, 3}) as sent_packets:
            ...
            self.router.handle_rx(packet2, in_port=3)
    ...
    def test_drop(self):
        ...
        with self._ensure_no_send(all_ports={1, 2, 3}):
            self.router.handle_rx(packet, in_port=2)

#    def test_infinity(self):
#        """Ensures we don't forward using routes with latency of infinity."""
#        h3 = _create_host("h3")
#        self.router.table.update({
#            h3: TableEntry(
#                destination=h3, port=1,
#                latency=self.INFINITY, expire_time=FOREVER,
#            )
#        })
#        packet = Packet(dst=h3, src=_create_host("hz"))
#        with self._ensure_no_send(all_ports={1, 2, 3}):
#            self.router.handle_rx(packet, in_port=3)
#
#    def test_bigger_than_infinity(self):
#        """Ensures we don't forward using routes with latency over infinity."""
#        h3 = _create_host("h3")
#        self.router.table.update({
#            h3: TableEntry(
#                destination=h3, port=1,
#                latency=self.INFINITY+1, expire_time=FOREVER,
#            )
#        })
#        packet = Packet(dst=h3, src=_create_host("hz"))
#        with self._ensure_no_send(all_ports={1, 2, 3}):
#            self.router.handle_rx(packet, in_port=3)
    ...
    def test_forward_hairpins(self):
        """Ensures hairpin packets are forwarded by default"""
        hs = _create_host("hs")

        with self._patch_send(all_ports={1, 2, 3}) as sent_packets:
            packet1 = DataPacket(dst=self.h2, src=hs, name="foo")
            self.router.handle_rx(packet1, in_port=3)

        self._assert_packets_sent(sent_packets, {
            1: [], 2: [], 3: [packet1],
        })

    def test_drop_hairpins(self):
        """Ensures hairpin packets are dropped when DROP_HAIRPINS set"""
        DVRouterBase._DVRouterBase__DROP_HAIRPINS = True
...
class TestHandleAdvertisement(TestDVRouterBase):
    ...
    def test_single_neighbor(self):
        ...
        self._advertise(h1, port=1, route_latency=2)
        self._advertise(h2, port=1, route_latency=0)
        self._advertise(h3, port=1, route_latency=0.5)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=7.5, expire_time=65),
            h2: TableEntry(destination=h2, port=1, latency=5.5, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=6, expire_time=65),
        })
    ...
    def test_multiple_neighbors(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._advertise(h1, port=1, route_latency=1)
        self._advertise(h2, port=1, route_latency=2)
        self._advertise(h3, port=1, route_latency=3)

        self._advertise(h2, port=3, route_latency=5)
        self._advertise(h3, port=3, route_latency=7)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5.5, expire_time=65),
            h2: TableEntry(destination=h2, port=3, latency=2 + 5, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=3 + 5.5, expire_time=65),
        })
    ...
    def test_ties(self):
        ...
        self._advertise(h1, port=3, route_latency=4)
        self._advertise(h2, port=3, route_latency=5)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5, expire_time=65),
            h2: TableEntry(destination=h2, port=1, latency=2 + 5, expire_time=65),
        })
    ...
    def test_update_same_port_shorter(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._advertise(h2, port=3, route_latency=1)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5.5, expire_time=65),
            h2: TableEntry(destination=h2, port=3, latency=2 + 1, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=3 + 5.5, expire_time=65),
        })
    ...
    def test_update_same_port_longer(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._advertise(h2, port=3, route_latency=5.25)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5.5, expire_time=65),
            h2: TableEntry(destination=h2, port=3, latency=2 + 5.25, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=3 + 5.5, expire_time=65),
        })
    ...
    def test_update_alternate_port_shorter(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._advertise(h2, port=1, route_latency=1)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5.5, expire_time=65),
            h2: TableEntry(destination=h2, port=1, latency=1 + 5.5, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=3 + 5.5, expire_time=65),
        })
    ...
    def test_update_alternate_port_longer_same_port(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._advertise(h2, port=3, route_latency=6)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=1 + 5.5, expire_time=65),
            h2: TableEntry(destination=h2, port=3, latency=2 + 6, expire_time=65),
            h3: TableEntry(destination=h3, port=1, latency=3 + 5.5, expire_time=65),
        })
...
class TestRemoveRoutes(TestDVRouterBase):
    ...
    def test_expire_routes_none(self):
        ...
        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=8, expire_time=109),
            h2: TableEntry(destination=h2, port=3, latency=5, expire_time=115),
        })
    ...
    def test_expire_routes_forever(self):
        ...
        self._assert_table_equal({
            h3: TableEntry(destination=h3, port=10, latency=1, expire_time=FOREVER),
        })
    ...
    def test_expire_routes_more(self):
        ...
        with self._patch_send(all_ports={1, 2, 3, 10}) as sent_packets:
            self.router.handle_rx(packet, in_port=10)
        ...
        expected = TableEntry(destination=h2, port=3, latency=5, expire_time=115)
    ...
    def test_do_not_send_routes_when_expiring(self):
        """
        Check that send_routes() isn't called from expire_routes()

        Doing this is not necessary.
        """
        old_send_routes = self.router.send_routes
        def bad (*args, **kw):
          self.fail("Don't call send_routes() from expire_routes()")
        self.router.send_routes = bad

        try:
          self._set_current_time(110)
          self.router.expire_routes()
        finally:
          self.router.send_routes = old_send_routes
...
class TestPoisonReverse(TestDVRouterBase):
    ...
    def test_poison_reverse(self):
        ...
        self._assert_route_ads_sent(
            ads, {
                1: {Route(self.h1, self.INFINITY), Route(self.h2, 5)},
                2: {Route(self.h1, 8), Route(self.h2, 5)},
                3: {Route(self.h1, 8), Route(self.h2, self.INFINITY)},
                10: {Route(self.h1, 8), Route(self.h2, 5)},
            }
        )
...
class TestInfiniteLoops(TestDVRouterBase):
    ...
    def test_propagate_poison(self):
        ...
        h2 = self.h2
        h1 = self.h1
        self._advertise(route_dst=h2, route_latency=1, port=3)

        with self._patch_send_routes(all_ports={1, 2, 3, 10}, allow_dup=True) as ads:
            self._advertise(route_dst=h2, port=3, route_latency=self.INFINITY)
            self.router.send_routes(force=True, single_port=None)
        self._assert_route_ads_sent(ads, {
            1: {Route(h2, self.INFINITY), Route(h1, self.INFINITY)},
            2: {Route(h2, self.INFINITY), Route(h1, 8)},
            3: {Route(h2, self.INFINITY), Route(h1, 8)},
            10: {Route(h2, self.INFINITY), Route(h1, 8)},
        })
    ...
    def test_handle_poison_no_route(self):
        ...
        self._advertise(route_dst=h1, port=1, route_latency=self.INFINITY)
        self._advertise(route_dst=h1, port=3, route_latency=self.INFINITY)
    ...
    def test_handle_poison_no_route_unpoison(self):
        ...
        self._advertise(route_dst=h1, port=1, route_latency=1)
...
class TestRoutePoisoning(TestDVRouterBase):
    ...
    def setUp(self):
        ...
        self._advertise(h2, port=1, route_latency=6)
        self._set_current_time(96)
        self._advertise(h3, port=1, route_latency=9)
        self._set_current_time(98)
        self._advertise(h4, port=2, route_latency=1)
        self._set_current_time(100)
        self._advertise(h2, port=3, route_latency=9)

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=8, expire_time=107),
            h2: TableEntry(destination=h2, port=1, latency=11, expire_time=109),
            h3: TableEntry(destination=h3, port=1, latency=14, expire_time=111),
            h4: TableEntry(destination=h4, port=2, latency=2, expire_time=113),
        })

    def _assert_poison_sent(self, host, ads, port):
        """Asserts that poison for a host has been sent to a port."""
        expected = Route(host, self.INFINITY)
    ...
    def test_expire_routes_single(self):
        ...
        self._assert_route_ads_sent(ads, {
            1: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, 2)},
            2: {Route(h1, self.INFINITY), Route(h2, 11),
                Route(h3, 14), Route(h4, self.INFINITY)},
            3: {Route(h1, self.INFINITY), Route(h2, 11),
                Route(h3, 14), Route(h4, 2)},
            10: {Route(h1, self.INFINITY), Route(h2, 11),
                 Route(h3, 14), Route(h4, 2)},
        })
    ...
    def test_expire_routes_multiple(self):
        ...
        self._assert_route_ads_sent(ads, {
            1: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            2: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            3: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            10: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                 Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
        })
    ...
    def test_expire_routes_multiple_periodic_poison(self):
        ...
        expected = {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                    Route(h3, self.INFINITY), Route(h4, self.INFINITY)}
        self._assert_route_ads_sent(ads, {
    ...
    def test_expire_routes_multiple_periodic_poison_unpoison(self):
        ...
        h1, h2, h3, h4 = self.h1, self.h2, self.h3, self.h4
        self._advertise(h1, port=2, route_latency=1)

        ads = self._gather_timer_ads(ports={1, 2, 3, 10}, advance=5)
        self._assert_route_ads_sent(ads, {
            1: {Route(h1, 2), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            2: {Route(h1, self.INFINITY), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            3: {Route(h1, 2), Route(h2, self.INFINITY),
                Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
            10: {Route(h1, 2), Route(h2, self.INFINITY),
                 Route(h3, self.INFINITY), Route(h4, self.INFINITY)},
        })

class TestLinkDownTriggeredUpdates(TestDVRouterBase):
    """
    Tests for updates triggered by link down.
    ...
    def _test_handle_link_down_update_multiple(self, expected_advertisements):
        ...
        h3 = self.h3
        self._advertise(h3, port=1, route_latency=1)
        self._advertise(h3, port=2, route_latency=10)
        ...
        if not self.router.POISON_ON_LINK_DOWN:
            with self._patch_send_routes(all_ports={2, 3, 10}, allow_dup=True) as ads:
                self.router.handle_link_down(port=1)
        ...
        self._assert_route_ads_sent(ads, expected_advertisements, ignore_extra=True)
    ...
    def test_handle_link_down_update_multiple_poison(self):
        ...
        h1, h2, h3 = self.h1, self.h2, self.h3
        self._test_handle_link_down_update_multiple(expected_advertisements={
            2: {Route(h1, self.INFINITY), Route(h3, self.INFINITY), Route(h2, 5)},
            3: {Route(h1, self.INFINITY), Route(h3, self.INFINITY), Route(h2, self.INFINITY)},
            10: {Route(h1, self.INFINITY), Route(h3, self.INFINITY), Route(h2, 5)},
        })
    ...
    def test_handle_link_down_update_poison(self):
        ...
        self._test_handle_link_down_update(expected_advertisements={
            1: set(),
            2: {Route(h1, self.INFINITY), Route(h2, 5)},
            3: {Route(h1, self.INFINITY), Route(h2, self.INFINITY)},
            10: {Route(h1, self.INFINITY), Route(h2, 5)}
        })

    def test_link_down_advertise_poison_multiple(self):
        """Ensures that poison ads are sent when triggered (multiple rounds)."""
        self.poison_set_up()
        h1, h2, h3, h4 = self.h1, self.h2, self.h3, self.h4

        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads1:
            self.router.send_routes(force=True)
        with self._patch_send_routes(all_ports={1, 2, 3, 10}, allow_dup=True) as ads2:
            self.router.handle_link_down(port=2)
        for k,v in ads2.items():
            ads1[k].update(v)
        ads = ads1
        ads.pop(2,None)
        ...
        with self._patch_send_routes(all_ports={3, 10}, allow_dup=True) as ads:
            self.router.handle_link_down(port=1)

        self._assert_route_ads_sent(ads, {
            3: {Route(h1, self.INFINITY), Route(h2, self.INFINITY), Route(h3, self.INFINITY)},
            10: {Route(h1, self.INFINITY), Route(h2, self.INFINITY), Route(h3, self.INFINITY)},
        }, ignore_extra=True)

class TestLinkUpTriggeredUpdates(TestDVRouterBase):
    """
    Tests for updates triggered by link up.
    ...
    def poison_set_up(self, send_on_link_up):
        """
        Sets up configuration before each unit test in this stage:
        ...
        """
        self._set_up(poison_reverse=True, poison_expired=True,
                     poison_on_link_down=True, split_horizon=False, send_on_link_up=send_on_link_up, triggered=True)
    ...
    def test_handle_link_up(self):
        """Ensure advertisements are sent immediately on link up."""
        self.poison_set_up(send_on_link_up=True)
        h1, h2, h3, h4 = self.h1, self.h2, self.h3, self.h4
    ...
    def test_no_handle_link_up(self):
        """Ensure advertisements are NOT sent immediately on link up."""
        self.poison_set_up(send_on_link_up=False)
        h1, h2, h3, h4 = self.h1, self.h2, self.h3, self.h4
...
class TestIncrementalUpdates(TestDVRouterBase):
    """
    ...
    def _test_non_shortest_route_longer(self):
        ...
        # self.router.send_routes(force=True)
        with self._ensure_no_send(all_ports={1, 2, 3, 10}):
            self._advertise(h1, port=2, route_latency=self.INFINITY)
    ...
    def _test_shortest_route_slightly_longer(self):
        h2 = self.h2
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads:
            self._advertise(h2, port=3, route_latency=3)
    ...
    def test_shortest_route_longer_poison(self):
        ...
        self.assert_full_poisoning()
        self._test_shortest_route_longer(expected_advertisements={
            1: {Route(h2, 12), Route(h1, self.INFINITY)},
    ...
    def _test_non_shortest_route_slightly_shorter(self):
        h2 = self.h2
        with self._ensure_no_send(all_ports={1, 2, 3, 10}):
            self._advertise(h2, port=1, route_latency=5)
    ...
    def _test_non_shortest_route_shorter(self, expected_advertisements):
        h1 = self.h1
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads:
            self._advertise(h1, port=3, route_latency=1)
    ...
    def _test_new_host(self, expected_advertisements):
        ...
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads:
            self._advertise(h3, port=2, route_latency=1)
    ...
    def test_consecutive_poison(self):
        ...
        with self._patch_send_routes(all_ports={1, 2, 3}) as ads:
            self._advertise(h1, port=1, route_latency=3)
        self._assert_route_ads_sent(ads, {1: {Route(h1, self.INFINITY)}, 2: {Route(h1, 8)},
                                          3: {Route(h1, 8)}})

        self._assert_table_equal({
            h1: TableEntry(destination=h1, port=1, latency=8, expire_time=65),
        })

        with self._patch_send_routes(all_ports={1, 2, 3}) as ads:
            self._advertise(h2, port=2, route_latency=6)
        self._assert_route_ads_sent(ads, {1: {Route(h2, 7)},
                                          2: {Route(h2, self.INFINITY)}, 3: {Route(h2, 7)}})
    ...
    def test_expire_no_duplicate_with_poison(self):
        ...
        h1, h2 = self.h1, self.h2
        self._assert_route_ads_sent(ads, {
            1: {Route(h1, self.INFINITY), Route(h2, 5)},
            2: {Route(h1, self.INFINITY), Route(h2, 5)},
            3: {Route(h1, self.INFINITY), Route(h2, self.INFINITY)},
            10: {Route(h1, self.INFINITY), Route(h2, 5)},
        })
    ...
    def test_poison_alternate_route(self):
        ...
        self._advertise(h1, port=1, route_latency=self.INFINITY)
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads:
            self._advertise(h1, port=3, route_latency=10)
    ...
    def test_poison_no_update(self):
        ...
        with self._ensure_no_send(all_ports={1, 2, 3, 10}):
            self._advertise(h1, port=3, route_latency=self.INFINITY)
    ...
    def test_link_down_advertise_poison_multiple2(self):
        """
        Ensures that poison ads are sent when triggered (multiple rounds).

        This is similar to a test in the link-triggered stage, but now
        we ensure that no extra routes are sent
        """
        self.poison_set_up()
        h1, h2, h3, h4 = self.h1, self.h2, self.h3, self.h4
    ...
    def test_link_down_advertise_poison_multiple(self):
        ...
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads1:
            self.router.send_routes(force=True)
        with self._patch_send_routes(all_ports={1, 2, 3, 10}) as ads2:
            self.router.handle_link_down(port=2)
        for k,v in ads2.items():
            ads1[k].update(v)
        ads = ads1
...
def _format_ads (ads):
  r = []
  r.append("== Advertisements ==")
  for k,vs in ads.items():
    r.append(str(k))
    for v in vs.values():
      r.append("  " + str(v))
  return "\n".join(r)
...
def main():
    test_cases = (
        ...
        TestSplitHorizon, TestInfiniteLoops, TestPoisonReverse,
        TestRoutePoisoning, TestLinkDownTriggeredUpdates,
        TestLinkUpTriggeredUpdates, TestIncrementalUpdates
    )
    ...
    parser.add_argument("--ref", "-r", default=0, type=int,
                        help="allows testig a reference implemention")
    ...
    if not args.ref:
      from dv.router import DVRouter
    if args.ref == -1:
      from dv.skeleton import DVRouter
    elif args.ref == 1:
      from dv.router_ref1 import DVRouter
    elif args.ref == 2:
      from dv.router_ref2 import DVRouter
    ...
    #max_per_stage = 100.0/(len(test_cases) - 1)

    def pct (n, fmt="%.2f"):
      return "{: >6}%".format(fmt % (n,))

    score = 0
    max_score = 0
    eprint("Results:")
    complete = True
    for stage,test_case in enumerate(test_cases[1:], 1):
        if stage > args.step_num:
            complete = False
            #eprint("  Stage {0: >2} {1: <31}:  +0.00% :  tests not run".format(
            eprint("  Stage {0: >2} {1: <31}:  tests not run".format(
                stage, test_case.__name__)
            )
            continue
        ...
        stage_score = test_case.SCORE * float(successes[stage]) / totals[stage]
        score += stage_score
        ...
        #eprint("  Stage {0: >2} {1: <31}: {5} : {2: >2} / {3: <2} passed{4}".format(
        eprint("  Stage {0: >2} {1: <31}: {2: >2} / {3: <2} passed{4}".format(
            stage, test_case.__name__, successes[stage], totals[stage],
            msg) #, pct(stage_score, "%+.2f"))
        )
        #score += max_per_stage * float(successes[stage]) / totals[stage]
    eprint()
    #eprint("Total score: %.2f / %.2f" % (score, 100)) #10 * (len(test_cases) - 1)))
    #eprint("Completion: %.2f%%" % (score,))
    #eprint("Partial Score: %.2f%%" % (score/max_score * 100.0,))
    #eprint("Total Score:   %.2f%%" % (score,))
    ex = ""
    if not complete:
      ex = "  "
      ex += " " if args.step_num >= 10 else ""
      eprint("Up to stage %s: %s" % (args.step_num, pct(score/max_score * 100.0)))
    eprint("Total Score: %s%s" % (ex, pct(score)))
