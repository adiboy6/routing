...
def launch (switch_type = sim.config.default_switch_type, host_type = sim.config.default_host_type,
            switches = 6, hosts = 4, links = 8, latency = 1, multiple_hosts = True, seed = None):
    """
    Creates a random fully-connected (single component) topology

    The topology will have *switches* switches, and *hosts* hosts.
    The switches will be connected by *links* (not counting any extra links
    which connect hosts to switches).
    ...
    for u,v in sorted(links):
      switches[u].linkTo(switches[v], latency=latency)
    ...
    for i in range(h):
      ...
      switch.linkTo(host, latency=latency)
